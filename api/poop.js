...
router.put('/update/:id', async (req, res) => {
  try {
    const updatePoops = await Poops.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
...
router.post('/5poops', async (req, res) => {
  try {
    const mostRecentPoops = await Poops.find({ email: req.body.email })
      .sort({ date: 1 })
      .limit(5);
...
router.post('/allPoops', async (req, res) => {
  try {
    const allPoops = await Poops.find({ email: req.body.email }).sort({
      date: 1,
    });
